import { Award, QrCode, ShieldCheck, Ticket } from "lucide-react";
...
export function DonorVouchersPage() {
  ...
  const [selectedVoucher, setSelectedVoucher] =
    useState<DonationVoucher | null>(null);
  ...
  if (vouchersQuery.isPending) {
    return (
      <DonorPageFrame
        ...
        description={t(
          "donor.voucherNotice",
          "Official priority blood replacement certificates issued for verified donations.",
        )}
      >
        <LoadingState
          label={t("common.loadingRecords", "Loading donation vouchers…")}
        />
      </DonorPageFrame>
    );
  }
      ...
      actions={
        <Button asChild variant="secondary" size="sm">
          <Link to="/donor/donations">
            {t("nav.donationHistory", "Donation History")}
          </Link>
        </Button>
      }
    >
        ...
        >
          <div className="flex items-start gap-3">
            ...
            <div className="space-y-1">
              ...
              >
                {t(
                  "healthcare.voucherCertificate",
                  "About Priority Blood Replacement Vouchers",
                )}
              </h2>
              <p className="text-xs text-muted-foreground leading-relaxed">
                {t(
                  "donor.voucherNotice",
                  "For every successful whole blood or apheresis donation, the Ministry of Health issues a digital voucher guaranteeing priority, zero-fee replacement blood units for you or your first-degree family members in any registered hospital.",
                )}
              </p>
            </div>
          </div>
        ...
        {vouchers.length === 0 ? (
          <EmptyState
            ...
            description={t(
              ...
              "Complete a blood donation to earn priority replacement vouchers.",
            )}
            action={
              <Button asChild size="sm">
                <Link to="/donor/requests">
                  {t("nav.donationRequests", "Browse Shortage Requests")}
                </Link>
              </Button>
            }
          />
          ...
          <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
            {vouchers.map((voucher) => (
              <article
                key={voucher.id}
                className={`relative overflow-hidden rounded-xl border bg-surface p-5 sm:p-6 shadow-2xs transition-colors ${
              ...
              >
                ...
                <div className="mt-4 space-y-3 text-xs">
                  ...
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">
                      {t("hospital.created", "Issued Date")}:
                    </span>
                  ...
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">
                      {t("bloodBank.expiry", "Valid Through")}:
                    </span>
                    <span className="font-semibold text-foreground font-mono">
                ...
                <div className="mt-6 flex justify-end">
                  ...
                  >
                    ...
                    <span>
                      {t("donor.redeemVoucher", "Inspect Certificate")}
                    </span>
                  </Button>
                </div>
              </article>
            ...
            <DialogContent className="max-w-md">
              <DialogHeader>
                <DialogTitle className="flex items-center gap-2 text-base font-bold">
                  ...
                  {t(
                    "healthcare.voucherCertificate",
                    "Priority Replacement Voucher",
                  )}
                </DialogTitle>
                <DialogDescription>
                  {t(
                    "donor.voucherNotice",
                    "Digital Certificate of Blood Donation and Beneficiary Rights",
                  )}
                </DialogDescription>
              </DialogHeader>
              ...
              <div className="space-y-4 py-3 text-xs">
                <div className="rounded-lg border border-border bg-surface-subtle p-4 text-center">
                  ...
                  <p className="mt-2 text-[10px] text-muted-foreground">
                    {t(
                      "bloodBank.scannableNotice",
                      "Scan for national transfusion network verification",
                    )}
                  </p>
                </div>
                ...
                <dl className="space-y-2.5 divide-y divide-border">
                  <div className="flex justify-between pt-2">
                    <dt className="text-muted-foreground">
                      {t("donor.donationsTitle", "Donation Reference")}
                    </dt>
                  ...
                  <div className="flex justify-between pt-2">
                    <dt className="text-muted-foreground">
                      {t("hospital.facility", "Issue Authority")}
                    </dt>
                  ...
                  <div className="flex justify-between pt-2">
                    <dt className="text-muted-foreground">
                      {t("common.timeline", "Validity Period")}
                    </dt>
                    <dd className="font-semibold text-foreground font-mono">
                      <bdi dir="ltr">{selectedVoucher.issuedDate}</bdi> →{" "}
                      <bdi dir="ltr">{selectedVoucher.expiryDate}</bdi>
                    </dd>
                  </div>
                  <div className="pt-2">

...
async def initiate(
    ...
    return await svc.initiate_payment(data, current)
...
async def webhook(
    ...
    received_hmac = hmac or x_paymob_hmac or ""
...
async def list_request(blood_request_id: str, current: CurrentUser, svc: PaymentService = Depends(get_payment_service)):
    return await svc.list_for_request(blood_request_id, current)

@router.patch("/{payment_id}", response_model=PaymentPublic, dependencies=[Depends(require_roles(Role.ADMIN, Role.PLATFORM_SUPPORT))])
async def update(payment_id: str, data: PaymentUpdate, current: CurrentUser, svc: PaymentService = Depends(get_payment_service)):
    return await svc.update(payment_id, data, current)

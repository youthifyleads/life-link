...
class SQLAlchemyDonorRepository(DonorRepository):
    ...
    async def create(self,r): self.session.add(DonorModel(donor_id=r.id,user_id=r.user_id,blood_type=r.blood_type,date_of_birth=r.date_of_birth,governorate=r.governorate,eligibility_status=r.eligibility_status,last_donation_date=r.last_donation_date,latitude=r.latitude,longitude=r.longitude)); await self.session.commit(); return r
    async def update(self,r):
        o=(await self.session.execute(select(DonorModel).where(DonorModel.donor_id==r.id))).scalar_one(); o.blood_type=r.blood_type; o.date_of_birth=r.date_of_birth; o.governorate=r.governorate; o.eligibility_status=r.eligibility_status; o.last_donation_date=r.last_donation_date; o.latitude=r.latitude; o.longitude=r.longitude; await self.session.commit(); return r
    ...
    async def list_responses_for_request(self,req_id): return [DonationResponseRecord(o.response_id,o.response_date,o.status,o.notes,o.blood_request_id,o.donor_id) for o in (await self.session.execute(select(DonationResponseModel).where(DonationResponseModel.blood_request_id==req_id).order_by(DonationResponseModel.response_date.desc()))).scalars().all()]
    ...
    async def list_vouchers(self, donor_id):
        stmt = (select(DonationVoucherModel)
                .join(DonationModel, DonationModel.donation_id == DonationVoucherModel.donation_id)
                .where(DonationModel.donor_id == donor_id)
                .order_by(DonationVoucherModel.issued_at.desc()))
        rows = (await self.session.execute(stmt)).scalars().all()
        return [DonationVoucherRecord(o.voucher_id, o.voucher_number, o.issued_at, o.status, o.donation_id) for o in rows]

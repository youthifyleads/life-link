...
- Official Blood Bag QR clients encode the opaque QR payload returned by `/blood-bags/{id}/qr`; `/blood-bags/scan` resolves it server-side to the real bag, current status, and movement history. The older request-tracking QR remains only for backward compatibility.
...
- OTP development mode is short-lived, single-use and attempt-limited; signup/password-reset verification uses email; SMS/TextBee is out of scope.
